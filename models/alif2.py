...
class AdLIF2(Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.weight = Parameter(
            torch.empty((self.out_features*2, self.in_features), **factory_kwargs)
        )
        self.bias = Parameter(torch.empty(self.out_features*2, **factory_kwargs))
        ...
        def step_fn(recurrent, alpha, beta, thr, a, b, u_rest, carry, cur):
            ...
            u_cur = cur[:, :self.out_features]
            w_cur = cur[:, self.out_features:]
            ...
            w = beta * w_tm1 + (1-beta) * (a * u + b * z + w_cur) * self.q
            return (u, z, w), z
        self.step = step_fn
    ...
    def forward(
        ...
        new_states, z_t = self.step(self.recurrent, decay_u, decay_w, self.thr, self.a, self.b, self.u0, states, current)
