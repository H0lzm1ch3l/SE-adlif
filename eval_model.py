...
def adlif_pytorch_states_dict_to_jax_dict(ckpt_path):
    ...
    enc_l1_decay_u = get_decay(1.0, 
                               ckpt_state_dict['model.encoder.l1.tau_u_trainer.tau_min'], 
                               ckpt_state_dict['model.encoder.l1.tau_u_trainer.tau_max'],
                               ckpt_state_dict['model.encoder.l1.tau_u_trainer.weight'])


    enc_l2_decay_u = get_decay(1.0, 
                               ckpt_state_dict['model.encoder.l2.tau_u_trainer.tau_min'], 
                               ckpt_state_dict['model.encoder.l2.tau_u_trainer.tau_max'],
                               ckpt_state_dict['model.encoder.l2.tau_u_trainer.weight'])


    dec_l1_decay_u = get_decay(1.0, 
                               ckpt_state_dict['model.decoder.l1.tau_u_trainer.tau_min'], 
                               ckpt_state_dict['model.decoder.l1.tau_u_trainer.tau_max'], 
                               ckpt_state_dict['model.decoder.l1.tau_u_trainer.weight'])


    dec_l2_decay_u = get_decay(1.0, 
                               ckpt_state_dict['model.decoder.l2.tau_u_trainer.tau_min'], 
                               ckpt_state_dict['model.decoder.l2.tau_u_trainer.tau_max'],
                               ckpt_state_dict['model.decoder.l2.tau_u_trainer.weight'])

    
    lout_decay = get_decay(1.0, 
                           ckpt_state_dict['model.decoder.out_layer.tau_u_trainer.tau_min'], 
                           ckpt_state_dict['model.decoder.out_layer.tau_u_trainer.tau_max'], 
    ...
    if ckpt['hyper_parameters']['cfg']['cell'] != 'lif':
        ...
        enc_l1_decay_w = get_decay(1.0,
                                ckpt_state_dict['model.encoder.l1.tau_w_trainer.tau_min'], 
                                ckpt_state_dict['model.encoder.l1.tau_w_trainer.tau_max'], 
                                ckpt_state_dict['model.encoder.l1.tau_w_trainer.weight'])
        
        enc_l2_decay_w = get_decay(1.0, 
                                ckpt_state_dict['model.encoder.l2.tau_w_trainer.tau_min'], 
                                ckpt_state_dict['model.encoder.l2.tau_w_trainer.tau_max'], 
                                ckpt_state_dict['model.encoder.l2.tau_w_trainer.weight'])

        dec_l1_decay_w = get_decay(1.0, 
                                ckpt_state_dict['model.decoder.l1.tau_w_trainer.tau_min'], 
                                ckpt_state_dict['model.decoder.l1.tau_w_trainer.tau_max'], 
                                ckpt_state_dict['model.decoder.l1.tau_w_trainer.weight'])
        
        dec_l2_decay_w = get_decay(1.0, 
                                ckpt_state_dict['model.decoder.l2.tau_w_trainer.tau_min'], 
                                ckpt_state_dict['model.decoder.l2.tau_w_trainer.tau_max'], 
    ...
    ckpt_state_dict['loss.temp'] = jnp.maximum(temp*temp_decay**factor, min_temp)
...
def se_adlif_model(model, weights: dict[str, Array], inputs:Array):
    enc_l1_u0 = weights['model.encoder.l1.u0']
    enc_l1_w0 = jnp.zeros_like(enc_l1_u0)
    enc_l1_z0 = jnp.zeros_like(enc_l1_z0)
    ...
    def loop(carry, inputs):
        ...
        enc_l1_u = enc_l1_decay_u * enc_l1_u + (1.0 - enc_l1_decay_u) * (inputs - enc_l1_w)
        ...
        probs = jax.nn.softmax(lout_u/weights['loss.temp'], -1)
        out = inverse_A_law(jnp.sum(probs * weights['loss.bin_edges'], keepdims=True))
        out = out.astype(jnp.float32)
        return (enc_l1_u, enc_l1_w, enc_l1_z, enc_l2_u, enc_l2_w, enc_l2_z, dec_l1_u, dec_l1_w, dec_l1_z, dec_l2_u, dec_l2_w, dec_l2_z, lout_u), out
    _, out = jax.lax.scan(loop, (enc_l1_u0, enc_l1_w0, enc_l1_z0, enc_l2_u0, enc_l2_w0, enc_l2_z0, dec_l1_u0, dec_l1_w0, dec_l1_z0, dec_l2_u0, dec_l2_w0, dec_l2_z0, lout_u),
                 inputs, unroll=1)
    return out
...
def main(cfg: DictConfig):
    py_dataloader = create_test_libri_dataloader(cfg.ckpt_path, 1, 0)
    weight = adlif_pytorch_states_dict_to_jax_dict(cfg.ckpt_path)
    adlif_dataloder_loop(cfg.model, weight, py_dataloader, cfg.save_to)
