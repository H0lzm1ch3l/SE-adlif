    ...
    ckpt_state_dict.update(
        {
            ...
            "q": q, "nb_bottleneck_neurons": nb_bottleneck_neurons,"thr": thr}
        )
...
def se_adlif_model(weights: dict[str, Array], inputs:Array):
    ...
    def loop(carry, inputs):
        ...
        return (enc_l1_u, enc_l1_w, enc_l2_u, enc_l2_w, enc_l2_z, dec_l1_u, dec_l1_w, dec_l1_z, dec_l2_u, dec_l2_w, dec_l2_z, lout_u), out
...
def adlif_dataloder_loop(weights, py_dataloader, save_to):
    save_to = Path(save_to)
    save_to.mkdir(exist_ok=True, parents=True)
    model = partial(se_adlif_model, weights)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_to', help='path where evaluation results will be saved')
    parser.add_argument('--batch_size', type=int, default=1)
    args = parser.parse_args()
    print(jax.devices()[0])
    py_dataloader = create_test_libri_dataloader(args.ckpt_path, args.batch_size, 0)
    weight = adlif_pytorch_states_dict_to_jax_dict(args.ckpt_path)
    adlif_dataloder_loop(weight, py_dataloader, args.save_to)