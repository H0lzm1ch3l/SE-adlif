...
def adlif_pytorch_states_dict_to_jax_dict(ckpt_path):
    ...
    ckpt_state_dict['loss.temp'] = 10
    print(ckpt_state_dict['loss.temp'])
...
def adlif_dataloder_loop(model, weights, prediction_delay, wave_list_sorted, save_to):
    ...
    model = jax.jit(jax.vmap(model), donate_argnums=0,)
    i = 0
    m = ScaleInvariantSignalNoiseRatio()
    for wave in tqdm(wave_list_sorted, "evaluation"):
        _inputs, _ = torchaudio.load(wave)
        inputs = jnp.array(_inputs, dtype=jnp.float32,).T
        inputs = inputs[None, ...]
        inputs = jnp.concat((inputs, jnp.zeros((inputs.shape[0], prediction_delay, inputs.shape[2]), dtype=inputs.dtype)), axis=1)
        out = model(inputs)
        print(out.shape)
        # jnp.save(save_to/str(i), out[0, prediction_delay:], allow_pickle=False)
        print(m(torch.from_numpy(np.array(out[0, prediction_delay:])), _inputs.T))
        i += 1
